...
public class OrderService {
    ...
    private AccountResponse fetchAccount(int accountId) {
        try {
            ApiResponse<AccountResponse> response = identityClient.getAccountById(accountId);
            if (response != null) {
    ...
    public OrderResponse createOrder(OrderRequest request) throws ParseException {
        ...
        order.setAccountId(request.getAccount_id());
        order.setPaymentMethod(request.getPaymentMethod());
        Order saved = orderRepository.save(order);

        OrderResponse orderResponse = orderMapper.toOrderMapper(saved);
        orderResponse.setAccount(accountRes.getResult());
    ...
    public List<OrderResponse> getAllOrders() {
        return orderRepository.findAll().stream()
                .map(o -> {
                    OrderResponse res = orderMapper.toOrderMapper(o);
                    res.setAccount(fetchAccount(o.getAccountId()));
                    return res;
                })
    ...
    public OrderResponse updateOrderStatus(int orderId, StatusOrdering statusOrder) {
        ...
        OrderResponse res = orderMapper.toOrderMapper(updated);
        res.setAccount(fetchAccount(updated.getAccountId()));
        return res;
    }
    ...
    public List<OrderResponse> getOrdersByAccountId(int account_id) {
        List<Order> orders = orderRepository.findByAccountId(account_id).stream()
                .filter(order -> order.getStatusOrder() != StatusOrdering.CANCELLED).collect(Collectors.toList());
        
        AccountResponse accountResponse = fetchAccount(account_id);
        return orders.stream()
                .map(o -> {
                    OrderResponse res = orderMapper.toOrderMapper(o);
                    res.setAccount(accountResponse);
                    return res;
                })
                .collect(Collectors.toList());
    }
